...
class DropboxClient {
	...
	public function UploadFile($src_file, $dropbox_path='', $overwrite=true, $parent_rev=null)
	{
		...
		output("UPLOAD: File Size: " . $file_size, "info");					
		...
		{			
			...
			while(!feof($fh)) {				
				...
				if($this->useCurl) {			
					...
					output("UPLOAD: Chunk Size: " . $chunk_size . " (".$chunk_size_mb." Megabytes)", "info");
					output("UPLOAD: Offset: " . $offset . " (".$offset_mb." Megabytes)", "info");
					curl_setopt($context, CURLOPT_INFILESIZE, $chunk_size);
					$response = json_decode(self::execCurlAndClose($context));
					output("UPLOAD: Curl Execute.", "info");
					$remaining = $file_size-$offset;
					$remaining_mb = round((($remaining/1024)/1024), 2);
					output("UPLOAD: File Size Remaining: " . $remaining . " (".$remaining_mb." Megabytes)", "info");
					fseek($fh,$offset);
					if($offset >= $file_size)
						break;
				} else {
					output("UPLOAD: Not Using Curl.", "info");
					$content = fread($fh, self::UPLOAD_CHUNK_SIZE);
					output("UPLOAD: File File Size.", "info");
					$context = $this->createRequestContext($url, "PUT", $content);
					output("UPLOAD: Process File Upload.", "info");
