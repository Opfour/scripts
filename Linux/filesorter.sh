...
#
# 
# $Revision: 1.89 $ $Date: 2012/02/20 04:20:46 $
#
...
set GROUP {
    ...
    audio	{title nosfx prefix} { .aac .mp3 .m4a .m4b .m4p .aif .aiff .snd .flac .ogg .oga .mid .wav .wma .ra .mod }
...
array set EQ {
    ...
    DEVDIR {^lib$|^src|(^|[ _-])test|jar|dump}
...
proc series {l} {
	...
	if {$slen >= 3} {
	    ...
	    if {$relz && ![regexp $RX_RELZ $post]} {append post " " [kmg [lindex $tuple 10] post]}	;# maybe also do this if length of series >100 or would qualify by relsize percentage (set zlen [expr 100/$SWITCH(relsize)]), size of first tuple is representative but maybe instead show largest in series or sum
	    append post "..." $slen
	    lset tuple 9 $post
...
proc column {l gtitcnt} {
	...
	set ll $ll1
    } else {	;# if title at bottom then bump, swap +1 row for -1 col (easier to read, but don't drastically reshape)
...
#puts "col pfx: $pfxl for ${cols}x$rows <= $llen"
	...
	for {set j 0; set n $i} {$n<$llen} {incr j; incr n $rows} {
		...
		puts -nonewline [color $pre pre]
		if {$style!=""} {puts -nonewline [colorcode $style]}
...
proc main {argv {argi 0}} {
	...
	if {!$first} {puts ""}
	if {$fcnt > 1} {puts "$dir:"}
	sl2 $dir $g($dir) $g($dir) 0	;# no raw names
	set first 0
    }
    foreach dir $dirl {
	if {!$first} {puts ""}
